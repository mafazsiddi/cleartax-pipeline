...
export function assignmentEmail({ issue, statusName, assignee, assignedBy, appUrl }) {
  ...
  return {
    ...
    text: `${assignee}, you've been assigned "${issue.key} ${issue.title}"${by}. Status: ${statusName}. Priority: ${priority}.${issue.dueDate ? ` Due: ${issue.dueDate}.` : ''}${appUrl ? ` Open the board: ${appUrl}` : ''}\n\nThis is an automated message from an unmonitored address. Please don't reply.`,
    html: shell(`
      <p style="margin:0 0 8px 0;font-size:15px;color:#1B2333;font-weight:600;">
        Hi ${esc(assignee)}, an issue is yours
      </p>
      <p style="margin:0 0 20px 0;font-size:13.5px;color:#586074;line-height:1.5;">
        You were assigned an issue on the Mira board${esc(by)}.
      </p>
