...
/**
 * @param {object} params
 * @param {{key: string, title: string, description?: string, priority?: string, dueDate?: string}} params.issue
 * @param {string} params.statusName  resolved display name of the issue's current status (statuses are per-project now, not a fixed set)
 */
export function assignmentEmail({ issue, statusName, assignee, assignedBy, appUrl }) {
  const priority = PRIORITY_NAMES[issue.priority] || issue.priority || 'Medium';
  ...
  return {
    subject: `You've been assigned: ${issue.key} ${issue.title}`,
    text: `${assignee}, you've been assigned "${issue.key} ${issue.title}"${by}. Status: ${statusName}. Priority: ${priority}.${issue.dueDate ? ` Due: ${issue.dueDate}.` : ''}${appUrl ? ` Open card: ${appUrl}` : ''}\n\nThis is an automated message from an unmonitored address. Please don't reply.`,
    html: shell(`
      <p style="margin:0 0 8px 0;font-size:15px;color:#1B2333;font-weight:600;">
        Hi ${esc(assignee)}, a task is yours
      </p>
      <p style="margin:0 0 20px 0;font-size:13.5px;color:#586074;line-height:1.5;">
        You were assigned a task on the Mira board${esc(by)}.
      </p>

      <div style="padding:16px;border:1px solid #E6E9F0;border-radius:12px;background:#FBFBFD;">
        <p style="margin:0 0 2px 0;font-size:12px;font-weight:600;color:#8A93A6;letter-spacing:0.3px;">${esc(issue.key)}</p>
        <p style="margin:0 0 10px 0;font-size:15px;font-weight:600;color:#1B2333;line-height:1.4;">${esc(issue.title)}</p>
        ${issue.description ? `<p style="margin:0 0 12px 0;font-size:13px;color:#586074;line-height:1.55;">${esc(issue.description)}</p>` : ''}
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
          ${row('Status', statusName)}
          ${row('Priority', priority)}
          ${issue.dueDate ? row('Due', issue.dueDate) : ''}
      ...
      ${appUrl ? `<p style="margin:22px 0 0 0;">
        <a href="${esc(appUrl)}" style="display:inline-block;padding:11px 20px;border-radius:10px;background:${BRAND};
           color:#ffffff;font-size:13.5px;font-weight:600;text-decoration:none;">Open card</a>
      </p>` : ''}
    `),
  };
}
...
/**
 * @param {object} params
 * @param {{key: string, title: string}} params.issue
 * @param {string} params.mentionedBy display name of whoever wrote the comment
 * @param {string} params.commentText the comment, with mention tokens already resolved to plain @Name text
 */
export function mentionEmail({ issue, mentionedBy, commentText, appUrl }) {
  return {
    subject: `${mentionedBy} mentioned you in ${issue.key}`,
    text: `${mentionedBy} mentioned you in a comment on "${issue.key} ${issue.title}":\n\n${commentText}${appUrl ? `\n\nOpen card: ${appUrl}` : ''}\n\nThis is an automated message from an unmonitored address. Please don't reply.`,
    html: shell(`
      <p style="margin:0 0 8px 0;font-size:15px;color:#1B2333;font-weight:600;">
        ${esc(mentionedBy)} mentioned you
      </p>
      <p style="margin:0 0 20px 0;font-size:13.5px;color:#586074;line-height:1.5;">
        In a comment on <strong>${esc(issue.key)}</strong> — ${esc(issue.title)}
      </p>

      <div style="padding:16px;border:1px solid #E6E9F0;border-radius:12px;background:#FBFBFD;">
        <p style="margin:0;font-size:13.5px;color:#1B2333;line-height:1.55;white-space:pre-wrap;">${esc(commentText)}</p>
      </div>
